...
module.exports = router
    ...
    .post('/',checkAuth(), (req,res,next) => {
        ...
        const coordinateAddress = `${req.body.street1} ${req.body.street2} ${req.body.city} ${req.body.state} ${req.body.zipCode}`;
        geocoder.geocode(coordinateAddress)
            .then(res =>  req.body.coordinates = {latitude: res[0].latitude, longitude: res[0].longitude})
            .then(() => {
                const list = new Listing(req.body);
                list.generateDate();
                list.save()
